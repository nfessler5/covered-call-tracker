...
import (
	"context"
	positionssvr "covered-call-tracker/gen/http/positions/server"
...
func handleHTTPServer(ctx context.Context, u *url.URL, positionsEndpoints *positions.Endpoints, wg *sync.WaitGroup, errc chan error, dbg bool) {
	...
	// responses.
	var (
		positionsServer *positionssvr.Server
	)
	{
		eh := errorHandler(ctx)
		positionsServer = positionssvr.New(positionsEndpoints, mux, dec, enc, eh, nil)
	}

	// Configure the mux.
	positionssvr.Mount(mux, positionsServer)
	...
	for _, m := range positionsServer.Mounts {
